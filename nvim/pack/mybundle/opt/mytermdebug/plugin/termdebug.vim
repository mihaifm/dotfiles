...
func s:InitHighlight()
  ...
  hi default debugButtons ctermfg=gray guifg=gray
endfunc
...
func s:InstallWinbar(force)
  if has('menu') && &mouse != ''
    let bar = ""
    let pad = "\\ \\ \\ \\ "
    let bar = bar .. "%#debugButtons#%@Over@" .. pad
    let bar = bar .. "%#debugButtons#%@Step@" .. pad
    let bar = bar .. "%#debugButtons#%@Finish@" .. pad
    let bar = bar .. "%#debugButtons#%@Continue@" .. pad
    let bar = bar .. "%#debugButtons#%@Stop@" .. pad
    let bar = bar .. "%#debugButtons#%@Evaluate@" .. pad

    exe "setlocal winbar=" .. bar
