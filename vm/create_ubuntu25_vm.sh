...
SSH_PUBKEY_FILE="${SSH_PUBKEY_FILE:-${USER_HOME}/.ssh/ubuntu.pub}"

# allocate next available VMID
VMID="${VMID:-$(pvesh get /cluster/nextid)}"
VMNAME="${VMNAME:-Ubuntu${VMID}}"
...
if [[ ! "$MEMORY_GB" =~ ^[1-9][0-9]*$ ]]; then
  die "MEMORY_GB must be a positive number of GB"
fi
MEMORY_MB="$(( MEMORY_GB * 1024 ))"

if [[ ! "$MEMORY_MIN_GB" =~ ^[1-9][0-9]*$ ]]; then
  die "MEMORY_MIN_GB must be a positive number of GB"
fi
...
[ubuntu_vms]
ubuntu${VMID} ansible_host=${ip} ansible_user=ubuntu ansible_ssh_private_key_file=~/.ssh/ubuntu ansible_python_interpreter=/usr/bin/python3
EOF
